...
export default function QuestionForm({ topics, lessons }: { topics: Topic[]; lessons: Lesson[] }) {
  ...
  async function createQuestion() {
    ...
    if (type === "mcq") {
      const opts = [optA, optB, optC, optD].map((label, i) => ({
        question_id: q.id,
        label,
      ...
      // store the correct short answer inside solution_explainer for now (simple v1)
      // Better later: dedicated field + tolerant matching rules.
      if (!solution) {
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <label className="block">
          ...
          <select className="mt-1 w-full rounded-md border p-2" value={lessonId} onChange={(e) => setLessonId(e.target.value)}>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <label className="block">
          ...
          <select className="mt-1 w-full rounded-md border p-2" value={type} onChange={(e) => setType(e.target.value as any)}>
      ...
      <label className="block">
        ...
        <textarea className="mt-1 w-full rounded-md border p-2 min-h-[90px]" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
      </label>
      ...
      <label className="block">
        ...
        <textarea className="mt-1 w-full rounded-md border p-2 min-h-[70px]" value={solution} onChange={(e) => setSolution(e.target.value)} />
      </label>
      ...
      {type === "mcq" ? (
        <div className="rounded-md border p-3 space-y-2">
          ...
          <label className="block">
            ...
            <input className="mt-1 w-full rounded-md border p-2" type="number" value={correctIndex} onChange={(e) => setCorrectIndex(Number(e.target.value))} />
          </label>
        </div>
      ) : (
        <div className="rounded-md border p-3 space-y-2">
          <div className="text-sm font-semibold">Short answer</div>
          <input className="w-full rounded-md border p-2" placeholder="Correct answer (exact match for v1)" value={shortAnswer} onChange={(e) => setShortAnswer(e.target.value)} />
